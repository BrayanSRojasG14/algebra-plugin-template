...
library PoolInteraction {
    function getSqrtPrice(
        IAlgebraPool pool
    ) internal view returns (uint160 price) {
    ...
    function getPluginConfig(
        IAlgebraPool pool
    ) internal view returns (PluginConfig pluginConfig) {
        uint8 _config;
    ...
    {
        (price, currentTick, fee, pluginConfig, , ) = IAlgebraPool(pool)
            .globalState();
    }

    function getPreviousAndNextTicks(
        IAlgebraPool pool
    ) internal view returns (int24 previousTick, int24 nextTick) {
        previousTick = IAlgebraPool(pool).prevTickGlobal();
        nextTick = IAlgebraPool(pool).nextTickGlobal();
    }

    function changePluginConfigIfNeeded(
        IAlgebraPool pool,
